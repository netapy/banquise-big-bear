<script>
  ...
  const uploadAsset = async () => {
    ...
    clientprov.setEndpoint("https://devback.banquise.app/v1").setProject("62e0fcfca3b3c5124f16");
    await accountprov.createEmailSession('contact@banquise.app', localStorage.getItem("clef").substring(0,10));

    for (var fileIndex in imageList){
        if(imageList[fileIndex] != null){
            try {
              const result = await storageprov.createFile('62e0fd376bc80b0bcf70', "BQZ-" +file_id + fileIndex, imageList[fileIndex]);
              console.log(result);
            } catch (error) {
              console.error(error);  
            }
...
<div class="p-3 mb-3">
  ...
  <div class="p-3 d-flex flex-column align-items-center mb-4 mt-3" style="background-color: #FAFAFA; border-radius:30px; max-width:1000px">
    ...
    <div class="p-3 d-flex flex-wrap" style="max-width:800px;">
      <input class="p-1 m-1" type="text" placeholder="File id" bind:value={file_id} />
      <input class="p-1 m-1" type="text" placeholder="Public name" bind:value={asset_name} />
      <input class="p-1 m-1" type="text" placeholder="Credits" bind:value={asset_credits} />
      <input class="p-1 m-1" type="text" placeholder="Category (split by ';' no space)" bind:value={asset_category} />
    ...
    <button class=" hvr-grow" style="width: fit-content; font-size:1.2rem" on:click={uploadAsset}
     disabled={file_id ==null || asset_name ==null || file_id ==null || asset_type ==null || imageList[0] == null}>
      Send to server
    </button>
  </div>
