<script>
  ...
  const buildTable = async () => {
    const gamesDataReq = await databases.listDocuments("62e0fd281976e7171db9", "62e0fe08a4a5d6592df2", [query.limit(100), query.orderDesc("$updatedAt")]);
    idMap = await getUserNames(gamesDataReq.documents.map((x) => x["owner"]));
    gamesData = gamesDataReq["documents"];
    grid = new Grid({
      autoWidth: true,
      resizable: true,
      search: true,
  ...
  const filterPublicGames = (e) => {
    if (e.target.checked) {
      grid.updateConfig({
        data: gamesData.filter(x=>x["public"] != false).map((game) => [
          game.$id,
          game["title"],
          idMap[game["owner"]],
          game["$createdAt"].split("T")[0],
...
<div class="p-3">
  <h3>All games</h3>
  <div class="p-3">
    <input type="checkbox" class="me-2" id="publicIO" on:change={filterPublicGames} /><label for="publicIO"
      >Only public games</label
    >
  </div>
  <div id="tableWrapper" />
</div>
<Modals>
  <div slot="backdrop" class="backdrop" on:click={closeModal} on:keydown={closeModal} transition:fade />
</Modals>
